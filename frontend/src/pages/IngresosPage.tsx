...
export default function IngresosPage() {
  ...
  const load = () => api.get(`/ingresos?mes=${mes}`).then(setItems);
  useEffect(() => { load(); }, [mes]);
    ...
    <div className="max-w-3xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Total del mes: <span className="text-success font-semibold">{fmt(total)}</span></p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        {items.length === 0 ? (
          <p className="text-center text-gray-400 py-12">Sin ingresos este mes</p>
        ) : (
            ...
            <tbody className="divide-y divide-gray-50">
              {items.map(i => (
                <tr key={i.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3 text-right text-success font-semibold">{fmt(i.monto)}</td>
      ...
      {modal.open && (
        <TransaccionModal tipo="ingreso" item={modal.item} onClose={() => setModal({ open: false })} onSave={() => { setModal({ open: false }); load(); }} />
      )}
    </div>
  );
}
